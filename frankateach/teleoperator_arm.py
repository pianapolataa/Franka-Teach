...
# Get the teleop state (Pause or Continue)
    ...
    def _rotate_frame(self, angle_rad, wrist_state):
        # rotate frame around the palm normal (y-axis)
    ...
    def _fix_quat_flip(self, quat):
        """Ensure quaternion continuity relative to previous command."""
        if not hasattr(self, "_last_quat"):
            self._last_quat = quat.copy()
            return quat

        # if q and -q: choose the closer one
    ...
    def _apply_retargeted_angles(self) -> None:
        ...
        if self.start_teleop:
            ...
            hand_angles[2] = self._scale_angle(hand_angles[2], -5, 5)   # palm normal
            hand_angles[0] = self._scale_angle(hand_angles[0], -10, 0)  # side axis
            hand_angles[1] = 0
